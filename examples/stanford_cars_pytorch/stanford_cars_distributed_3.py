...
def main():
    parser = argparse.ArgumentParser(
        description="Distributed Stanford Cars training with DeltaFabric"
    )
    parser.add_argument(
    ...
    try:
        full_train_dataset = datasets.StanfordCars(
        ...
        print(f"\nNode {node_id}: Error loading dataset: {e}")
        print("\nPlease run the provided download script to get the data:")
        print("  ./download_dataset.sh\n")
        return

    print(
        f"Train samples: {len(full_train_dataset)}, Test samples: {len(test_dataset)}"
    )
    ...
    with open(f"accuracy_dist3_node_{node_id}_alpha_{config.alpha}.txt", "w") as f:
